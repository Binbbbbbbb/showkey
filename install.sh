...
echo "==> 安装图标 → ${ICON_DIR}/showkey.svg"
install -Dm644 "${PROJECT_DIR}/icon/showkey.svg" "${ICON_DIR}/showkey.svg"

# 应用快捷方式（.desktop 启动项）：始终安装 / 覆盖，保证图标与快捷方式跟源码同步
...
echo "==> 安装应用图标 → ${ICON_THEME_DIR}/showkey.svg"
install -Dm644 "${PROJECT_DIR}/icon/showkey.svg" "${ICON_THEME_DIR}/showkey.svg"

echo "==> 安装应用快捷方式 → ${DESKTOP_FILE}"
